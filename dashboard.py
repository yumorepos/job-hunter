...
def main():
    ...
    with col1:
        ...
        # Create pie chart
        fig_pie = px.pie(
            values=source_counts.values, names=source_counts.index, hole=0.4, template="plotly_dark"
        )
    ...
    with col2:
        ...
        st.info(
            "💡 Status tracking not yet implemented. All jobs are currently tracked as 'Found'."
        )
    ...
    if search_query:
        mask = filtered_df["title"].str.contains(search_query, case=False, na=False) | filtered_df[
            "description"
    ...
    display_df = filtered_df.head(20)[
        ["title", "company", "location", "source", "scraped_at", "url"]
    ]
